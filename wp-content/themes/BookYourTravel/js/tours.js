...
(function ($) {
  ...
  bookyourtravel_tours = {
    init: function () {
      if (
        $(".tour-booking-form-calendar .booking_form_controls_holder").length >
        0
      ) {
        ...
        $(".radio").on("click.uniform", function (ignore) {
        ...
        bookyourtravel_tours.populateAvailableTourDates(
          bookyourtravel_tours.checkPreselectedTourDateAndPopulateAccordingly,
          true,
          true
        );
      }
      ...
      if (
        $("[data-location-id]").length > 0 &&
        $("[data-min-price-type='tour']").length > 0
      ) {
      ...
      if ($("[data-tour-id]").length > 0) {
        ...
        $.each(tourItems, function () {
          if (!$(this).hasClass("skip-ajax-call")) {
            ...
            if (
              $("[data-tour-id='" + tourId + "'] .price .amount").length > 0
            ) {
              ...
              $.ajaxQueue(
                {
                  ...
                  success: function (data) {
                    if (data !== undefined && data !== "") {
                      ...
                      $("[data-tour-id='" + tourId + "'] .price .amount").html(
                        bookyourtravel_scripts.formatPriceOnly(price)
                      );
                    } else {
    ...
    initBookingRequestObject: function () {
      if (
        ...
        window.bookingRequest === undefined ||
        window.bookingRequest.length === 0
      ) {
        window.bookingRequest = {};
      }
    ...
    bindTourDropDowns: function () {
      var maxCountOffset =
        window.bookingRequest.maxAllowedPeople -
      ...
      for (i = 1; i <= maxAdultCount; i += 1) {
        $(
          "<option " +
            (i === window.bookingRequest.adults ? "selected" : "") +
            ">"
        )
          .val(i)
          .text(i)
          .appendTo("#booking_form_adults");
      }
      ...
      $("#booking_form_adults").on("change", function (ignore) {
        if (parseInt($(this).val()) !== window.bookingRequest.adults) {
          ...
          window.bookingRequest.people =
            window.bookingRequest.adults + window.bookingRequest.children;
      ...
      for (j = 0; j <= maxChildrenCount; j += 1) {
        $(
          "<option " +
            (j === window.bookingRequest.children ? "selected" : "") +
            ">"
        )
          .val(j)
          .text(j)
          .appendTo("#booking_form_children");
      }
    ...
    checkPreselectedTourDateAndPopulateAccordingly: function () {
      if (
        window.bookingRequest.requestedDateFrom &&
        window.availableTourDates &&
        window.availableTourDates.indexOf(
          window.bookingRequest.requestedDateFrom
        ) >= 0
      ) {
        var dateFrom = bookyourtravel_scripts.convertLocalToUTC(
          new Date(window.bookingRequest.requestedDateFrom)
        );
        var timeFrom = Date.UTC(
          dateFrom.getFullYear(),
          dateFrom.getMonth(),
          dateFrom.getDate()
        );
        bookyourtravel_tours.selectTourDate(
          timeFrom,
          window.bookingRequest.requestedDateFrom
        );
      }
    ...
    populateAvailableTourDates: function (
      callDelegate,
      skipLoaderStart,
      skipLoaderEnd
    ) {
    ...
    resetTheForm: function () {
      ...
      if (
        ...
        window.bookingRequest.selectedTime > 0
      ) {
      ...
      $(".reservation_total").html(
        bookyourtravel_scripts.formatPrice(
          window.bookingRequest.totalTourOnlyPrice
        )
      );
      $(".extra_items_total").html(
        bookyourtravel_scripts.formatPrice(
          window.bookingRequest.extraItemsTotalPrice
        )
      );
      $(".total_price").html(
        bookyourtravel_scripts.formatPrice(window.bookingRequest.totalPrice)
      );
      $(".confirm_total_price_p").html(
        bookyourtravel_scripts.formatPrice(window.bookingRequest.totalPrice)
      );
    ...
    showTourBookingForm: function () {
      ...
      var adults =
        window.bookingRequest.adults !== undefined &&
        window.bookingRequest.adults
          ? window.bookingRequest.adults
          : "1";

      var children =
        window.bookingRequest.children !== undefined &&
        window.bookingRequest.children
          ? window.bookingRequest.children
          : "0";
      ...
      $(".booking_form_reservation_total_p").html(
        bookyourtravel_scripts.formatPrice(
          window.bookingRequest.totalTourOnlyPrice
        )
      );
      $(".booking_form_extra_items_total_p").html(
        bookyourtravel_scripts.formatPrice(
          window.bookingRequest.extraItemsTotalPrice
        )
      );
      $(".booking_form_total_p").html(
        bookyourtravel_scripts.formatPrice(window.bookingRequest.totalPrice)
      );
        ...
        $("#tour-booking-form").validate({
          ...
          invalidHandler: function (ignore, validator) {
            ...
            $(".booking-section").animate(
              {
                scrollTop:
                  $("#tour-booking-form div.error-summary").offset().top - 100,
              },
              500
            );
          },
    ...
    bindTourDatePicker: function () {
      if (window.minTourDate instanceof Date) {
        var firstDayOfMonth = new Date(
          window.minTourDate.getFullYear(),
          window.minTourDate.getMonth(),
          1
        );
        ...
        $(".booking_form_datepicker").datepicker({
          ...
          beforeShowDay: function (d) {
            ...
            if (window.availableTourDates) {
              ...
              if (
                $.inArray(dateTextToCompare, window.availableTourDates) === -1
              ) {
    ...
    processBooking: function () {
      if (window.bookingRequest !== undefined) {
        $(
          ".booking_form_controls_holder, .booking-commands .book-tour-proceed"
        ).hide();
        $("#booking-form-calendar .loading").parent().show();
        ...
        if ($(".confirm_reservation_total_p").length > 0) {
          $(".confirm_reservation_total_p").html(
            bookyourtravel_scripts.formatPrice(
              window.bookingRequest.totalTourOnlyPrice
            )
          );
        ...
        if ($(".confirm_extra_items_total_p").length > 0) {
          $(".confirm_extra_items_total_p").html(
            bookyourtravel_scripts.formatPrice(
              window.bookingRequest.extraItemsTotalPrice
            )
          );
    ...
    buildRatesTable: function () {
      ...
      if (selectedTime) {
        ...
        $(".reservation_total").html(
          bookyourtravel_scripts.getSmallLoaderHtml()
        );
    ...
    buildRatesTableStructure: function () {
      ...
      footerRow +=
        ...
        (colCount - 1) +
        "'>" +
        window.priceTotalLabel +
        "</th>";
      footerRow +=
        "<td class='reservation_total'>" +
        bookyourtravel_scripts.formatPrice(0) +
        "</td>";
      footerRow += "</tr>";
    ...
    buildRateRow: function (tourTime) {
      var tourDate = bookyourtravel_scripts.convertLocalToUTC(
        new Date(tourTime)
      );
      var theDate =
        ...
        success: function (data) {
          if (data !== undefined && data !== "") {
            ...
            if (price_data && price_data.regular_price) {
              window.bookingRequest.maxAllowedPeople =
                price_data.running_available_total - price_data.booked_places;
              bookyourtravel_tours.bindTourDropDowns();

              tableRow += "<tr>";
              tableRow +=
                "<td>" +
                $.datepicker.formatDate(window.datepickerDateFormat, tourDate) +
                "</td>";
              ...
              if (!window.tourIsPricePerGroup) {
                ...
                tableRow +=
                  "<td>" +
                  bookyourtravel_scripts.formatPrice(pricePerPeriod) +
                  "</td>";
                tableRow += "<td>" + window.bookingRequest.children + "</td>";
                tableRow +=
                  "<td>" +
                  bookyourtravel_scripts.formatPrice(childPricePerPeriod) +
                  "</td>";
              }

              if (!window.tourIsPricePerGroup) {
                totalPricePerPeriod =
                  pricePerPeriod * adults + childPricePerPeriod * children;
              } else {
              ...
              tableRow +=
                "<td>" +
                bookyourtravel_scripts.formatPrice(totalPricePerPeriod) +
                "</td>";
              ...
              bookyourtravel_extra_items.recalculateExtraItemTotals(
                window.bookingRequest.totalTourOnlyPrice
              );
              ...
              if (
                $("table.booking_price_breakdown").data("tablesorter") ===
                  null ||
                $("table.booking_price_breakdown").data("tablesorter") ===
                  undefined
              ) {
              ...
              $(".total_price").html(
                bookyourtravel_scripts.formatPrice(
                  window.bookingRequest.totalPrice
                )
              );

              if (window.enableDeposits) {
                window.bookingRequest.depositAmount =
                  window.bookingRequest.totalPrice *
                  (window.depositPercentage / 100);
                window.bookingRequest.depositDifference =
                  window.bookingRequest.totalPrice -
                  window.bookingRequest.depositAmount;
                ...
                if ($(".deposits_row").length > 0) {
                  ...
                  if (window.depositPercentage < 100) {
                    $(".deposit-info").html(
                      window.depositInfo.format(
                        window.depositPercentage,
                        bookyourtravel_scripts.formatPrice(
                          window.bookingRequest.depositDifference
                        )
                      )
    ...
    getSelectedTourDate: function () {
      if ($("#tour_date").val()) {
        return bookyourtravel_scripts.convertLocalToUTC(
          new Date(parseInt($("#tour_date").val()))
        );
      }
      return null;
    },
