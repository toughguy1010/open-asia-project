...
if (isset($_REQUEST['d']) && $_REQUEST['d'] == '1' && isset($_REQUEST['fesid'])) {
    ...
    $booking = $bookyourtravel_cruise_helper->get_cruise_booking($booking_id);
    if ($booking) {
        $cruise = new BookYourTravel_Cruise($booking->cruise_id);
        if ($cruise) {
...
$fs_table_extra_field_values = $bookyourtravel_cruise_helper->list_cruise_bookings($paged, $posts_per_page, 'Id', 'DESC', null, $booking_user_id, $is_partner_page ? $current_author_id : null);
