...
class BookYourTravel_Tour extends BookYourTravel_Entity
{
	...
	public function get_formatted_address($include_location = true) {
		...
		if (!empty($address)) {
			$address .= apply_filters('bookyourtravel_tour_address_location', ', ' . $location_part);
		} else {
			$address = apply_filters('bookyourtravel_tour_address_location', $location_part);
		}
	...
	public function get_type_is_repeated() {
		...
		return is_array($term_meta) && isset($term_meta['tour_type_is_repeated']) ? intval($term_meta['tour_type_is_repeated']) : 0;
	}
	...
	public function get_is_price_per_group() {
		return intval($this->get_custom_field( 'is_price_per_group' ));
	}
	...
	public function get_type_day_of_week_indexes() {
		...
		if ($type_is_repeated == 3) {
			$type_id = $this->get_type_id();
			$term_meta = get_option( "taxonomy_$type_id" );
			if (is_array($term_meta) && isset($term_meta['tour_type_is_repeated'])) {
	...
	public function get_tour_durations() {
		return wp_get_post_terms( $this->get_id(), 'tour_duration', array( "fields" => "all" ) );
	}	
	...
	public function get_field_value($field_name, $use_prefix = true) {
		if ( $field_name == 'facilities' || $field_name == 'facility' ) {
			...
			return $tag_ids;
		} else if ( $field_name == 'tour_duration' ) {
			$tag_ids = array();
			$tags = $this->get_tour_durations();
			if ($tags && count($tags) > 0) {
				for( $i = 0; $i < count($tags); $i++) {
					$tag = $tags[$i];
					$tag_ids[] = $tag->term_id;
