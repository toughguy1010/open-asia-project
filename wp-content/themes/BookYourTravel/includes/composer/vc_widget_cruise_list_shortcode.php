<?php
/**
 * BookYourTravel_VC_Byt_Widget_Cruise_List_Shortcode class
 *
 * @package WordPress
...
class BookYourTravel_VC_Byt_Widget_Cruise_List_Shortcode extends BookYourTravel_BaseSingleton {
  ...
  public function init() {
		if ( class_exists('Vc_Manager') ) {
			add_action( 'vc_before_init', array($this, 'byt_widget_cruise_list_shortcode_vc' ));

			add_action( 'wp_ajax_vc_bookyourtravel_get_cruise_tag_ids', array(
				$this,
				'get_cruise_tag_ids_ajax',
			) );
  ...
  function vc_backend_editor_render() {
	...
	function get_cruise_tag_ids_ajax(){
	...
	function byt_widget_cruise_list_shortcode_vc() {
		...
		$cruise_types = get_categories( array(
		...
		$cruise_type_params = array();
		$cruise_type_params[__('No filter', 'bookyourtravel')] = '';
		foreach ( $cruise_types as $cruise_type ) {
			$label = $cruise_type->name;
			$term_id = $cruise_type->term_id;
			$cruise_type_params[$label] = $term_id;
		}

		$cruise_durations = get_categories( array(
		...
		$cruise_duration_params = array();
		$cruise_duration_params[__('No filter', 'bookyourtravel')] = '';
		foreach ( $cruise_durations as $cruise_duration ) {
			$label = $cruise_duration->name;
			$term_id = $cruise_duration->term_id;
			$cruise_duration_params[$label] = $term_id;
		}		
		...
		$byt_widget_cruise_list_settings = array(
			'name'            => __('Cruise List', 'bookyourtravel'),
			'base'            => 'byt_widget_cruise_list',
			"description" => __( "Display a list of cruises", "bookyourtravel" ),
			...
			'params'          => array(
				array(
					...
					"param_name" => "title",
					'holder' => 'div',
					'std' => __('Explore our latest cruises', 'bookyourtravel'),
					'save_always' => true,
				),
				array(
					"type" => "dropdown",
				...
				array(
					"type" => "checkbox",
					"heading" => __("Show featured cruises only?", 'bookyourtravel'),
				...
				array(
					"type" => "checkbox",
					"heading" => __("Cruise tag ids", 'bookyourtravel'),
					"param_name" => "cruise_tag_ids",
					'holder' => 'div',
					'value' => array('empty' => 'empty'),
					'dependency' => array(
						'callback' => 'vcBookYourTravelCruiseTagsDependencyCallback'
					),
					'save_always' => true,
				),
				array(
					"type" => "dropdown",
					"heading" => __("Cruise type", 'bookyourtravel'),
					"param_name" => "cruise_type_ids",
					'holder' => 'div',
					"value" => $cruise_type_params,
					'save_always' => true,
				),
				array(
					"type" => "dropdown",
					"heading" => __("Cruise duration", 'bookyourtravel'),
					"param_name" => "cruise_duration_ids",
					'holder' => 'div',
					"value" => $cruise_duration_params,
					'save_always' => true,
				),				
				array(
					'type' => 'css_editor',
            ...
            "custom_markup" => sprintf("
                ...
                __("Cruise List", "bookyourtravel"),
                __("This is a cruise list widget. To access all the features of this block please use the front editor.", "bookyourtravel")
            )
		);

		vc_map(
			$byt_widget_cruise_list_settings
		);
	}
}

$bookyourtravel_vc_byt_widget_cruise_list_shortcode = BookYourTravel_VC_Byt_Widget_Cruise_List_Shortcode::get_instance();;
$bookyourtravel_vc_byt_widget_cruise_list_shortcode->init();

// A must for container functionality, replace Wbc_Item with your base name from mapping for parent container
if(class_exists('WPBakeryShortCode')){
    class WPBakeryShortCode_byt_widget_cruise_list extends WPBakeryShortCode {
