<?php
/**
 * BookYourTravel_VC_Byt_Widget_Accommodation_List_Shortcode class
 *
 * @package WordPress
...
class BookYourTravel_VC_Byt_Widget_Accommodation_List_Shortcode extends BookYourTravel_BaseSingleton {
  ...
  public function init() {
		if ( class_exists('Vc_Manager') ) {
			add_action( 'vc_after_init', array($this, 'byt_widget_accommodation_list_shortcode_vc' ));

			add_action( 'wp_ajax_vc_bookyourtravel_get_accommodation_tag_ids', array(
				$this,
				'get_accommodation_tag_ids_ajax',
            ) );
  ...
  function vc_backend_editor_render() {
	...
	function get_accommodation_tag_ids_ajax(){
		...
		$param = array(
			'param_name' => 'accommodation_tag_ids',
			'type' => 'checkbox',
		);
		$param_line = '';
		foreach ( $tags as $tag ) {
	...
	function byt_widget_accommodation_list_shortcode_vc() {
		...
		$accommodation_types = get_categories( array(
		...
		$accommodation_type_params = array();
		$accommodation_type_params[__('No filter', 'bookyourtravel')] = '';
		foreach ( $accommodation_types as $accommodation_type ) {
			$label = $accommodation_type->name;
			$term_id = $accommodation_type->term_id;
			$accommodation_type_params[$label] = $term_id;
		}
		...
		$byt_widget_accommodation_list_settings = array(
			'name'            => __('Accommodation List', 'bookyourtravel'),
			'base'            => 'byt_widget_accommodation_list',
			"description" => __( "Display a list of accommodations", "bookyourtravel" ),
			...
			'params'          => array(
				array(
					...
					"param_name" => "title",
					'holder' => 'div',
					'std' => __('Explore our latest accommodations', 'bookyourtravel'),
					'save_always' => true
				),
				array(
					"type" => "dropdown",
				...
				array(
					"type" => "checkbox",
					"heading" => __("Show featured accommodations only?", 'bookyourtravel'),
				...
				array(
					...
					"std" => 'title,image,actions,price,description,address,stars,rating',
					"description" => __( "Select which accommodation fields you want to show", "bookyourtravel" ),
					'save_always' => true,
				),
				array(
					"type" => "checkbox",
					"heading" => __("Accommodation tag ids", 'bookyourtravel'),
					"param_name" => "accommodation_tag_ids",
					'holder' => 'div',
					'value' => array('empty' => 'empty'),
					'dependency' => array(
						'callback' => 'vcBookYourTravelAccommodationTagsDependencyCallback'
					),
					'save_always' => true,
				),
				array(
					"type" => "dropdown",
					"heading" => __("Accommodation type", 'bookyourtravel'),
					"param_name" => "accommodation_type_ids",
					'holder' => 'div',
					"value" => $accommodation_type_params,
					'save_always' => true,
				),
				array(
					'type' => 'css_editor',
            ...
            "custom_markup" => sprintf("
                ...
                __("Accommodation List", "bookyourtravel"),
                __("This is an accommodation list widget. To access all the features of this block please use the front editor.", "bookyourtravel")
            )
		);

		vc_map(
			$byt_widget_accommodation_list_settings
		);
	}
}

$bookyourtravel_vc_byt_widget_accommodation_list_shortcode = BookYourTravel_VC_Byt_Widget_Accommodation_List_Shortcode::get_instance();
$bookyourtravel_vc_byt_widget_accommodation_list_shortcode->init();

// A must for container functionality, replace Wbc_Item with your base name from mapping for parent container
if(class_exists('WPBakeryShortCode')){
    class WPBakeryShortCode_byt_widget_accommodation_list extends WPBakeryShortCode {
