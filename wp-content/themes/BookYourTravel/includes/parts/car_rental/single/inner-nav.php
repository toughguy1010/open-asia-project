		...
		if (is_array($tab_array) && count($tab_array) > 0) {
			foreach ($tab_array as $tab) {
				if (isset($tab['id']) && (!isset($tab['hide']) || $tab['hide'] != '1')) {
					...
					if ($tab['id'] == 'description' || $tab['id'] == 'availability' || $tab['id'] == 'reviews') {
						if(empty($first_display_tab)) $first_display_tab = $tab['id'];
						BookYourTravel_Theme_Of_Custom::the_tab("car_rental", $tab_class, '',  sprintf('<a class="%s" href="#%s" title="%s">%s%s</a>', esc_attr($tab_icon_class), esc_attr($tab['id']), esc_attr($tab_label), $tab_icon_html, esc_html($tab_label)));
					} else {
						if(empty($first_display_tab)) $first_display_tab = $tab['id'];
