<?php
global $bookyourtravel_theme_globals, $post, $first_display_tab, $default_accommodation_tabs, $entity_obj, $layout_class, $tab;
...
$nav_layout = $bookyourtravel_theme_globals->get_accommodation_single_layout();

ob_start();		

get_template_part('includes/parts/accommodation/single/javascript', 'vars');
...
$layout_class = '';
if ($nav_layout == 'left') {
	get_template_part('includes/parts/accommodation/single/inner', 'nav');
...
$featured_element = $accommodation_obj->get_displayed_featured_element();
...
foreach ($tab_array as $tab) {
	if (isset($tab['id']) && (!isset($tab['hide']) || $tab['hide'] != '1')) {
		...
		if (count(BookYourTravel_Theme_Utils::custom_array_search($default_accommodation_tabs, 'id', $tab['id'])) == 0) {
			...
			if ($tab_has_fields) {
				get_template_part('includes/parts/accommodation/single/tab', 'content');
			}
		} else {
			get_template_part('includes/parts/accommodation/single/tab', 'content');		
		}
...
$content_and_description_match = $accommodation_obj->content_and_description_match();
